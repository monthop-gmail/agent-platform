...
**อัปเดต:** 2026-09-23 — **ข้อ 7 มีครึ่งที่สอง และมันจับของที่ครึ่งแรกจับไม่ได้** · Amendment 2 · 2026-09-20 — ข้อที่ใบนี้เปิดค้างไว้ มีคำตอบจากสองทีมที่ไม่ได้คุยกัน · Amendment 1
**Date:** 2026-09-20
...
## Amendment 2 — ข้อ 7 ผ่านได้ทั้งที่ตัวตรวจไม่เคยเห็น payload ชนิดนั้นเลย

ข้อ 7 ถาม *"ตัวตรวจของตัวเองครอบขอบเขตใหม่แล้วหรือยัง"* · `ecosystem-intelligence` ไปตอบข้อนี้ตามที่เราแจ้งไว้ที่ [`#27` ของเขา](https://github.com/monthop-gmail/ecosystem-intelligence/issues/27) แล้ว**ตอบผ่าน** — เขาผลิต payload รูปเดียวคือ `event/v1` และไม่ได้ pin `approval/v1` ขอบเขตใหม่จึงเท่ากับขอบเขตเดิม

บันทึกเป็น `leaf_scanner_scope` ในใบของเขา เพื่อแยก *ไม่มีอะไรต้องทำ* ออกจาก *ยังไม่ได้ดู* ตามที่ข้อ 8 ต้องการ

**แล้วการไปตอบข้อนี้ทำให้เขาเจอของ:**

> `guarantee` ข้อ 9 ของเรา **ไม่เคยเห็น `ECOSYSTEM_DRIFT_DETECTED` สักใบ**
>
> emitter ปล่อยเฉพาะ finding ระดับ `error` · guardian ไม่มี error ในสถานะปกติ · `$.metadata.detail` กับ `$.metadata.fix` จึงประกาศไว้ตั้งแต่ 17 ก.ย. **โดยไม่มีอะไรทาบกับของจริงเลย**

**ตัวตรวจอ่านถูก แต่ไม่เคยมีของให้อ่าน**

เขาแก้ด้วยการ**ยกป้าย `severity` ของ finding จริงจากรอบเดียวกัน ไม่ใช่เขียน fixture** — 13 ใบ/รอบ → **24 ใบ/รอบ** ผ่านทั้งหมด

### ทำไมครึ่งแรกจับไม่ได้ และครึ่งที่สองจับได้

```text
ข้อ 7 ครึ่งแรก   "ตัวตรวจครอบขอบเขตใหม่ไหม"       ← ตอบจากรายการชนิดที่เรา pin
ข้อ 7 ครึ่งหลัง   "ตัวตรวจเคยเห็นทุกชนิดที่เราผลิตไหม"  ← ตอบจาก payload ที่รันจริง
```

**สองรายการนี้ไม่เท่ากัน และส่วนต่างคือชนิดที่ประกาศไว้แล้วแต่ยังไม่เคยถูกผลิตในเส้นทางที่ตัวตรวจเดิน**

เป็นรูปเดียวกับบทเรียนของ [ADR-0031](0031-the-field-everyone-thought-was-someone-elses.md) ที่ว่าให้ไล่จาก *leaf ของ payload จริง* ไม่ใช่จาก *รายการฟิลด์ที่ตัวเองเขียน* — รอบนี้เลื่อนขึ้นไปอีกชั้น คือไล่จาก **ชนิดของ payload จริง** ไม่ใช่จากรายการชนิดที่ตัวเองประกาศ

### กฎที่เพิ่ม

```text
9. นอกจากถามว่าตัวตรวจครอบขอบเขตใหม่ไหม (ข้อ 7)
   ต้องถามด้วยว่า **ตัวตรวจเคยเห็น payload ครบทุกชนิดที่ตัวเองผลิตหรือยัง**
   — ชนิดที่ประกาศไว้แต่ไม่เคยถูกผลิตในเส้นทางที่ตัวตรวจเดิน
     ทำให้การประกาศนั้นเป็นเอกสารที่วางไว้ข้าง ๆ โดยที่เจ้าตัวไม่รู้
   — ปิดด้วยการทำให้ของจริงเดินผ่านเส้นทางนั้น ไม่ใช่ด้วยการเขียน fixture
     เพราะ fixture พิสูจน์ได้แค่ว่าตัวตรวจอ่านรูปนั้นออก ไม่ได้พิสูจน์ว่าของจริงมีรูปนั้น
```

### ข้อนี้ผูกกับ `platform_rules` ที่เราเพิ่งเขียนเมื่อ 20 ก.ย.

`event/v1` `v1.8.0` เพิ่มกฎว่า *การประกาศต้องเป็นสิ่งที่ตัว conformance check ของผู้ผลิตอ่านจริง ไม่ใช่เอกสารที่วางไว้ข้าง ๆ*

**เคสนี้ผ่านกฎนั้นทุกตัวอักษรแล้วยังเป็นเอกสารที่วางไว้ข้าง ๆ อยู่ดี** — เพราะกฎถามว่าตัวตรวจ*อ่าน*จากใบไหม ไม่ได้ถามว่ามันเคย*เจอ*ของที่ใบพูดถึงไหม · ข้อ 9 ปิดส่วนต่างนั้น
