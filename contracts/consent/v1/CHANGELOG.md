...
* `consent_rules` ข้อ 2 ขยายความว่า **"การใช้" บันทึกที่ไหน** — [ADR-0020](../../../decisions/0020-consent-event-vocabulary.md)

กฎเดิมบอกว่าการให้ · การใช้ · การเพิกถอน ต้องออก event ทุกครั้ง แต่ไม่ได้บอกว่าใช้ชื่ออะไรและบันทึกอย่างไร · ตอนนี้ระบุชัด:

| เหตุการณ์ | บันทึกด้วย |
| --- | --- |
| การให้ | `event_type: CONSENT_GRANTED` |
| การเพิกถอน | `event_type: CONSENT_REVOKED` |
| **การใช้** | **field `consent` บน event ที่เกิดการเข้าถึงจริง — ไม่มี event type ของตัวเอง** |

**กฎไม่ถูกลดทอน** — เพิ่มวิธีทำตามให้ชัด เพื่อไม่ให้คนอ่านเห็นสามคำแล้วไปตั้งชื่อ event type ที่สามขึ้นมาเอง ซึ่งจะทำให้มีสองบันทึกของเหตุการณ์เดียวกัน
