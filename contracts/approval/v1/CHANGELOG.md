...
คำกำกับที่ `authority` ว่ามันเป็น `Principal` ซึ่งพา `display_name` มาด้วย ([ADR-0034](../../../decisions/0034-semantics-1-5-leaf-declaration.md) Amendment 1)

[ADR-0031](../../../decisions/0031-the-field-everyone-thought-was-someone-elses.md) เลือกเขียนคำกำกับที่ต้นทางของฟิลด์ใน `identity/v1` **เพื่อให้คำเตือนเดินทางไปกับ `Principal` ทุกที่ที่มันถูกฝัง** — และมันเดินทางมาจริง

แต่ `care-agent-platform` รายงานว่า **ไม่ได้ไปมองเพราะยังไม่มีอะไรบอกให้มอง** · เขาขยายตัวไล่ leaf ให้ครอบใบอนุมัติหลัง RFC-0017 ขยายขอบเขตของกฎ leaf มาถึงสัญญาใบนี้ แล้ว **ฟ้อง `$.authority.display_name` ในรอบแรกที่รัน**

> เป็นรูปเดียวกับ `actor.display_name` ที่เราปิดไปเมื่อวันที่ 11 เป๊ะ ต่างกันแค่คราวนี้อยู่ในสัญญาคนละใบ

**คำเตือนที่เดินทางไปถึง ไม่เท่ากับคำเตือนที่ถูกอ่าน** — ฟิลด์ที่ `$ref` ออกไปจะถูกอ่านก็ต่อเมื่อมีคนตามลิงก์ไป · ใบนี้จึงเขียนซ้ำที่จุดใช้งาน พร้อมเหตุผลและหลักฐาน

### ไม่ breaking

เพิ่มคำอธิบาย · ไม่แตะ `$ref` ไม่แตะ `required`
