...
รับ invariant ของ `semantics_version` `1.3` **เข้าตัวสัญญา** — ต่อจาก `v1.7.1` ที่ขยับแค่ pointer และเขียนไว้เองว่าส่วนนี้ต้องมี ADR ก่อน ([ADR-0034](../../../decisions/0034-semantics-1-3-leaf-declaration.md))

### `guarantees` +1 — ข้อที่ 9

> leaf ที่อาจถือข้อความของคนต้องถูกประกาศไว้ในสัญญา **ที่เหลือทุก leaf เป็นตัวชี้**

ครั้งก่อนที่ `semantics_version` ขยับ (`v1.6.1` · RFC-0012) ไม่ต้องแก้ไฟล์เลยเพราะ `guarantees` ของเรา **ครอบ guarantee ใหม่อยู่แล้ว (8 ≥ 3)** · รอบนี้ไม่ถูกครอบ — ไม่มีข้อไหนใน 8 ข้อพูดถึง leaf

คัดถ้อยคำจากต้นทางทั้งท่อน รวม**ข้อจำกัดที่ต้นทางย้ายเข้ามาอยู่ในบรรทัดเดียวกับ invariant** — *บังคับได้เฉพาะฝั่งผู้ผลิต validator บอกได้ว่ามี key อะไร บอกไม่ได้ว่าใครพิมพ์ค่านั้น* · ร่างที่เรายื่นแยกข้อนี้ไว้คนละหัวข้อ **การย้ายของเขาถูกกว่า** และตรงกับหลักที่เราใช้ใน [ADR-0030](../../../decisions/0030-the-field-nobody-named.md) เอง

### `transition.reason` — leaf แรกที่ถูกประกาศ

เดิม `{ type: string }` **ไม่มีคำอธิบายเลยสักบรรทัด** — รูปเดียวกับ `error/v1.details` ([ADR-0030](../../../decisions/0030-the-field-nobody-named.md)) และ `identity/v1.display_name` ([ADR-0031](../../../decisions/0031-the-field-everyone-thought-was-someone-elses.md)) ซึ่งเป็น **สองใบที่เป็นเหตุให้ RFC-0013 เกิดขึ้นตั้งแต่แรก**

ประกาศแล้วว่าถือข้อความของคนได้ · เหตุที่เป็นตัวชี้ไม่ได้ · และ 🔒 **ผู้ผลิตต้องประกาศเองว่ากติกาการเก็บ/ลบอยู่ชั้นไหน** — `agent-platform` ประกาศแทนไม่ได้เพราะไม่ได้เป็นผู้เก็บ event ของใคร

### `platform_rules` +2

* การประกาศ leaf **ต้องเป็นสิ่งที่ตัว conformance check ของผู้ผลิตอ่านจริง** ไม่ใช่เอกสารข้าง ๆ — ข้อนี้อยู่ใน RFC ที่เรายื่นเอง ถ้าไม่ยกมาจะไม่มีที่ไหนใน contract ที่ consumer อ่านเจอ
* **`metadata` ไม่ได้ถูกยกเว้น** เพราะสัญญาไม่ได้ตั้งชื่อคีย์ในนั้น · ข้อห้าม private reasoning ที่มีอยู่แล้วเป็นคนละข้อ และ **ไม่ครอบข้อความของคนที่ไม่ใช่ของโมเดล**

### ไม่ breaking

ไม่มี field ใหม่ · ไม่แตะ `required` · ไม่แตะ `type` · payload ที่ valid กับ `v1.7.1` ยัง valid ทุกใบ

⚠️ **ผู้ผลิตได้ภาระใหม่ที่ validator ไม่ฟ้อง** — `care-agent-platform` กับ `devfactory-core` ประกาศ `text_fields` ของตัวเองแล้ว · **`ecosystem-intelligence` กับ `botforge` เป็นผู้ผลิต event เหมือนกันและยังไม่มีใครถาม**
