...
`platform_rules` +1 — **การประกาศแบบ wildcard กลืน ratchet ของตัวเอง ห้ามใช้**

`v1.8.0` เพิ่มกฎว่า `metadata` ไม่ได้ถูกยกเว้นจากกฎ leaf · ใบนี้ปิดรูปที่ตามมาทันที — **ผู้ผลิตที่ทำตามกฎนั้นโดยประกาศ `$.metadata.*` จะได้ ratchet ที่ไม่มีอะไรให้จับ** เพราะคีย์ใหม่ทุกตัวผ่านด้วยคำว่า *ประกาศไว้แล้ว*

ที่มาเป็นของ [`botforge`](https://github.com/monthop-gmail/botforge) ([#31](https://github.com/monthop-gmail/botforge/issues/31)) — **จับได้ตอนร่าง ไม่ใช่ตอนรัน** เพราะถามคำถามที่ไม่มีเครื่องมือไหนถามแทนได้:

> การประกาศนี้ทำให้ ratchet ของตัวเองเหลืออะไรให้จับ

เขียนเข้าสัญญาหลังของลงจริงแล้ว ไม่ใช่จากรายงาน — `platform-contract.yaml` บน `v2` ประกาศ 4 leaf ที่เป็นชื่อจริงทั้งหมด และปิด `metadata` ด้วยทะเบียนคีย์ในตัวตรวจแทน

### เป็นรูปที่สามของ false-pass ที่โต๊ะไล่ปิดกันทั้งสัปดาห์

```text
เครื่องมือล้ม ไม่บอก         →  ว่าง    (devfactory-core · | head -5 ที่ exit 0 เสมอ)
เดินไม่ถึงที่ที่ของอยู่         →  ว่าง    (เรา · binding check ที่ไม่เห็น enum ในชั้นซ้อน)
เดินถึง แต่ประกาศไว้ให้ผ่าน   →  เขียว   (ใบนี้)
```

**อันที่สามไม่ได้ว่างด้วยซ้ำ มันเขียว**

### ไม่ breaking

เพิ่มกฎในเอกสาร · ไม่แตะ `type` ไม่แตะ `required` ไม่มี field ใหม่
