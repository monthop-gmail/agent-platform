...
| 3b | **ไม่มีไฟล์ไหนมี key ซ้ำในระดับเดียวกัน** | YAML ที่ PyYAML ทับเงียบ ๆ แต่ YAML 1.2 ปฏิเสธ — schema ที่ผ่าน check เราแล้วพังที่ consumer ที่ใช้ parser อื่น ([#56](https://github.com/monthop-gmail/agent-platform/issues/56)) |
...
### ไฟล์ที่ `[5b]` เดิน — และทำไมถึงขยาย

เดิมเดินแต่ `planes/*.md` · `devfactory-core` ทักใน `dis-b492ed20` `seq 5` ว่า

> **"รอให้มีคนชน" เป็นกลยุทธ์ก็ต่อเมื่อมีคนที่จะชน** — ชั้นที่ `[5b]` ปิดคือ `planes/knowledge.md` ที่ค้าง 28 วัน เพราะ**ทีมที่จะชนมันคือทีมที่ไม่ได้อ่านเอกสารนั้น**
>
> การไล่ไฟล์ที่เหลือไม่ใช่การเดา มันคือการเอาเช็คที่เพิ่งเขียนไปรันกับไฟล์ที่ยังไม่เคยรัน — **ต้นทุนเกือบศูนย์ และไม่ต้องรอใคร**

ไล่แล้วพบว่า `README.md` อ้างว่า repo ไหนยังไม่มี **โดยไม่มี check ไหนเดินไปถึง** · ตอนนี้เดิน `planes/*.md` + `architecture/*.md` + `README.md` = 18 ไฟล์

⚠️ **`architecture/consumers.md` ถูกกันออกโดยเจตนา** — `[5]` เป็นเจ้าของไฟล์นั้นและใช้ anchor ที่ต้นบรรทัด · แถวที่นั่นมีหมายเหตุเชิงประวัติที่ *อ้างถึง* คำว่า *ยังไม่มี repo* ถึง 7 ที่ ซึ่งไม่ใช่คำกล่าวอ้าง

และผลของการเดินถูกพิมพ์ออกมาเป็นจำนวนไฟล์เสมอ ตามข้อที่ `devfactory-core` เสนอ — **ศูนย์จากการเดิน 18 ที่ กับศูนย์จากการไม่ได้เดิน เป็นคนละคำตอบ**

## [5c] `consumers.md` — ประโยคที่อ้างเวอร์ชัน แต่ไม่มีอะไรเทียบ

`check_derived` เทียบ pin ใน**ไฟล์ schema**กับต้นทาง · แต่ทะเบียนเขียนเป็น prose ไว้อีกที่หนึ่งว่า *"pin ปัจจุบัน `semantics_version: "…"` ตรงกับต้นทาง"* ซึ่งไม่มีอะไรมาเทียบ

**เกิดจริงแล้ว** — บรรทัดนั้นค้างอยู่ที่ `1.1` ข้าม `1.2` (21 ส.ค.) มาจนถึง `1.3` (17 ก.ย.) คือ **ผิดอยู่ 28 วัน** ในทะเบียนที่ผูกพัน โดยไม่มีอะไรฟ้อง — และเจอด้วยตาคนในวันเดียวกับ `planes/knowledge.md` ซึ่งเป็นรูปเดียวกันเป๊ะ

เทียบกับ pin ของ contract ที่ derive จริง **ไม่ใช่กับต้นทาง** — ถ้า pin ยังไม่ขยับ ประโยคก็ไม่ควรขยับ · สองอันนี้ต้องตรงกันเสมอไม่ว่าจะ conform อยู่หรือไม่


## [2] `last_verified` ในแถว ต้องตรงกับใบของเจ้าตัว

`check_registry` ดึง `platform-contract.yaml` ของทุก consumer อยู่แล้ว **และอ่าน `conformance.last_verified` มาตั้งแต่แรก** — แต่ใช้ถามข้อเดียวว่า *"`passing` แล้วมีวันไหม"* · **ไม่เคยถามว่าวันที่เราเขียนไว้ในแถวตรงกับที่เขาเขียนในใบไหม**

ผล: แถว `devfactory-core` ค้างที่ `2026-08-18` อยู่ **33 วัน** และแถว `ecosystem-intelligence` ค้าง **26 วัน** — เจอทั้งคู่ในรอบเดียวกัน โดยใบที่รายงานเข้ามารายงานแค่แถวของตัวเอง

⚠️ **ข้อที่ผู้รายงานเข้าใจต่างจากของจริง** — [#78](https://github.com/monthop-gmail/agent-platform/issues/78) เสนอไม่ให้เพิ่ม check ด้วยเหตุผลว่าต้องพึ่งเน็ตและพึ่ง repo คนอื่น ซึ่ง**จริงกับ `[5c]` แต่ไม่จริงกับ `[2]`** · `[2]` พึ่งเน็ตอยู่แล้วโดยนิยาม การเทียบข้อนี้จึงไม่เสียคำขอเพิ่มเลยสักครั้ง
...
| `README.md` อ้างว่า `botforge` ยังไม่มี repo | `plane: README.md:85: … แต่ monthop-gmail/botforge มีอยู่จริงแล้ว` — พิสูจน์ว่าเดินถึงไฟล์นอก `planes/` แล้ว |
| `architecture/consumers.md` มีคำว่า *ยังไม่มี repo* ในหมายเหตุเชิงประวัติ 7 ที่ | **ต้องไม่ FAIL** — ไฟล์นั้นเป็นของ `[5]` และถูกกันออกโดยเจตนา |
| `consumers.md` อ้าง `semantics_version: "1.1"` ทั้งที่ไฟล์ pin `1.3` | `regclaim: consumers.md อ้างว่า pin ปัจจุบันคือ ['1.1'] แต่ของจริง … คือ ['1.3']` — **เคสจริงที่ค้าง 28 วัน** |
| pin ในไฟล์ขยับเป็น `1.4` แต่ประโยคในทะเบียนไม่ขยับ | `regclaim: … ของจริง … คือ ['1.3', '1.4']` — พิสูจน์ว่าจับได้ทั้งสองทิศ |
| แถว `devfactory-core` เขียน `last_verified` `2026-08-18` แต่ใบของเขาเขียน `2026-09-20` | `registry: … ทะเบียนที่ผูกพันกำลังรายงานว่าตรวจครั้งล่าสุดเมื่อไหร่ผิด` — **เคสจริงที่ค้าง 33 วัน และพบอีกแถวที่ค้าง 26 วันพร้อมกัน** |
| แถวไม่มีคอลัมน์วันที่เลย | `registry: … แถวไม่มีคอลัมน์ last_verified ที่อ่านได้` — กันการลบคอลัมน์ทิ้งแล้วผ่าน |
