...
### แถวที่เขียนว่า "ยังไม่มี repo" — ยืนยันเมื่อ 2026-09-23 ด้วยเครื่องมือที่เห็น private

หกแถวข้างบน **ตรวจซ้ำด้วย `gh api` ซึ่งเห็น repo ที่เป็น private** — ทั้งหกได้ `404` จริง พร้อม `agent-gateway` ที่ `planes/gateway.md` อ้างถึง · `ecosystem-intelligence` ตรวจชุดเดียวกันด้วย token ของตัวเองแล้วได้ผลตรงกัน (`dis-d4412ed2` `seq 10`)

⚠️ **`conformance/drift_check.py` ทำข้อนี้ไม่ได้และจะทำไม่ได้ต่อไป** — [ADR-0011](../decisions/0011-conformance-automation.md) จำกัด host ไว้ที่ `raw.githubusercontent.com` ตัวเดียว ซึ่ง **ตอบ `404` ให้ repo ที่เป็น private เหมือนกับ repo ที่ไม่มีอยู่จริงทุกประการ**

`check_ghost_rows` จึงเป็นการตรวจ**ทางเดียว** โดยเจตนา — มันฟ้องได้เฉพาะตอนที่แถวบอกว่าไม่มีแต่เห็นว่ามี · **ตอนที่มันเงียบ แปลว่ายังไม่เจอ ไม่ได้แปลว่าไม่มี**

**ครึ่งที่เครื่องทำไม่ได้จึงต้องมีวันกำกับ** — ยืนยันครั้งล่าสุด `2026-09-23` · ถ้าแถวไหนเปลี่ยนเป็น repo จริงในรูปแบบ private เช็คจะไม่มีวันบอก

### หลักฐานว่าราคาของการเขียน "มีจริงไหม" ลงในทะเบียนคืออะไร

`it-operations-hub` ถูกบันทึกทั่วทั้ง ecosystem ว่าเป็นชื่อที่ไม่มี repo — **และ `created_at` ของมันคือ `2026-09-20`**

```text
2026-09-17  ตรวจด้วย gh ที่เห็น private  →  404  ถูกต้อง ณ วันนั้น
2026-09-20  repo ถูกสร้างขึ้นจริง         →  private
2026-09-22  ทะเบียนชื่อที่ร่างไว้ยังเขียนว่าไม่มี repo
```

**ไม่ใช่ความผิดพลาดของการมองไม่เห็น แต่เป็นสำเนาที่เก่า** — ต่อให้เครื่องมือเห็น private ตั้งแต่แรก แถวนั้นก็ยังผิดอยู่ดี เพราะข้อเท็จจริงเปลี่ยนหลังจากที่ถูกคัดลอกไปแล้ว

เป็นเหตุผลเดียวกับที่ทะเบียนใบนี้เลิกทำสำเนา `pinned_contracts_commit` และจำนวน event ([#78](https://github.com/monthop-gmail/agent-platform/issues/78))
